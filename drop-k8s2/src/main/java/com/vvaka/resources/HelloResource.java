...
public class HelloResource {
    ...
    @Path("/other")
    @GET
    public Response hello2() throws ExecutionException, UnknownHostException {

        final InetAddress ip = InetAddress.getLocalHost();
        final String hostname = ip.getHostName();
        Map<String, String> map = new HashMap<String, String>() {
            {
                put("msg", "Hello..... from drop-k8s2");
    ...
    @Path("/")
    @GET
    public Response hello() throws ExecutionException, InterruptedException {

        Map<String,Object> map = new LinkedHashMap<>();
        map.put("msg","Hello from Drop-K8s2 2222222222222");
        map.put("serviceUrl",otherAppUrl);
        map.put("##","########################################################");
        IntStream.range(0, 10).forEachOrdered(n -> {
            Client client2 = ClientBuilder.newClient();
            Map map2 = client2.target(otherAppUrl).path("/hello/other").request().get(Map.class);
